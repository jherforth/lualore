...
### Flying Mechanics
- Valkyries should hover smoothly when idle (bobbing up and down)
- They should NOT look like they're walking on air
- Wing flapping animation uses arm bone positions
- They should glide toward players when at medium range (8-25 blocks)
- They slow their fall when dropping (reduced fall velocity)
...
2. Right-click to use the wings
3. You'll get temporary flight:
   - Green Wings: 30 seconds, 10% speed boost
   - Blue Wings: 45 seconds, 20% speed boost
   - Violet Wings: 45 seconds, 35% speed boost
   - Gold Wings: 60 seconds, 50% speed boost
4. Wing particles trail behind you while flying
...
- Check debug.txt for "Strike failed - cooldown active" (normal) or "Strike function is nil!" (bug)
- Make sure Valkyrie has acquired you as target (watch for "Valkyrie acquired target" in logs)
